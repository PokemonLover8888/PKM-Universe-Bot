...
public class PokeTradeBotSV(PokeTradeHub<PK9> Hub, PokeBotState Config) : PokeRoutineExecutor9SV(Config), ICountBot, ITradeBot
{
    ...
    private async Task<PK9> ApplyAutoOT(PK9 toSend, TradeMyStatus tradePartner, SAV9SV sav, CancellationToken token)
    {
        ...
        // not a preserved file. Both guards below protect genuine transfers -- a real HOME tracker, or
        // a cross-generation file whose OT belongs to its original catcher -- and neither applies to
        // our own output. Without this, legendaries routed SwSh -> HOME -> PK9 shipped with the
        // generator's OT instead of the member's, because that path stamps a tracker and lands as
        // Generation 8 in Format 9.
        ...
        else
        {
            ...
            bool hasExplicitForeignLang = toSend.Language != configLanguage && toSend.Language >= 1 && toSend.Language <= 12;
            if (hasExplicitForeignLang)
            {
            ...
            // language is requested. SV may use HT_Language for the visual tag when CurrentHandler=1.
            if (hasExplicitForeignLang)
            {
        ...
        if (!tradeSV.Valid)
        {
            ...
            if (prevWordFilter)
            {
                ...
                if (laNoWordFilter.Valid)
                {
                    ...
                    await SetBoxPokemonAbsolute(BoxStartOffset, cln, token, null).ConfigureAwait(false);
                    return cln;
                }
            }
        }
        if (tradeSV.Valid)
        {
            ...
            // Pass null for sav to skip UpdateHandler — matches PLZA behavior which works for language tags
            await SetBoxPokemonAbsolute(BoxStartOffset, cln, token, null).ConfigureAwait(false);
            return cln;
        }
        else
